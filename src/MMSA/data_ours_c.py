...
class MMDataset(Dataset):
    def __init__(self, args, mode='train'):
        ...
        self.missing_v = args.get('missing_v', True) # Assuming default True
    ...
    def __init_mosi(self):
        ...
        reg_labels = data[self.mode]['regression_labels'].astype(np.float32)
        cls_lables = self.make_class_lable(reg_labels, self.num_classes)

        self.labels = {
            'M': reg_labels,
            'C': cls_lables,
            'missing_rate_l': np.zeros_like(reg_labels).astype(np.float32),
            'missing_rate_a': np.zeros_like(reg_labels).astype(np.float32),
            'missing_rate_v': np.zeros_like(reg_labels).astype(np.float32),
        }

        if self.datasetName == 'sims':
            for m in "TAV":
                self.labels[m] = data[self.mode]['regression_labels'+m]
        ...
        if self.do_missing:
            if self.mode == 'train':
                missing_rate = [np.random.uniform(size=(len(data[self.mode]['regression_labels']), 1)) for _ in range(3)]
                
                for i in range(3):
                    sample_idx = random.sample([i for i in range(len(missing_rate[i]))], int(len(missing_rate[i])/2))
                    missing_rate[i][sample_idx] = 0
                
                missing_rate[0] = missing_rate[0] if self.missing_l else np.zeros_like(missing_rate[0]) # debug
                # missing_rate[0] = np.ones_like(missing_rate[0])
                missing_rate[1] = missing_rate[1] if self.missing_a else np.zeros_like(missing_rate[1])
                missing_rate[2] = missing_rate[2] if self.missing_v else np.zeros_like(missing_rate[2])
                ...
                if self.missing_rate_eval_test == -1: # Test on a random missing rate.
                    missing_rate = [np.random.uniform(size=(len(data[self.mode]['regression_labels']), 1)) for _ in range(3)]
                else: # Test on a fixed missing rate.
                    missing_rate = [self.missing_rate_eval_test * np.ones((len(data[self.mode]['regression_labels']), 1)) for _ in range(3)]

                missing_rate[0] = missing_rate[0] if self.missing_l else np.zeros_like(missing_rate[0])
                missing_rate[1] = missing_rate[1] if self.missing_a else np.zeros_like(missing_rate[1])
                missing_rate[2] = missing_rate[2] if self.missing_v else np.zeros_like(missing_rate[2])
                ...
                self.labels['missing_rate_a'] = missing_rate[1]
                self.labels['missing_rate_v'] = missing_rate[2]
        else: # e.g., self.do_missing = False
            missing_rate = [np.zeros((len(data[self.mode]['regression_labels']), 1)) for _ in range(3)]
            self.labels['missing_rate_l'] = missing_rate[0]
            self.labels['missing_rate_a'] = missing_rate[1]
            self.labels['missing_rate_v'] = missing_rate[2]
    ...
    def make_class_lable(self, reg_labels, num_classes):
        if num_classes == 2:
            reg_labels_clip = np.clip(reg_labels, a_min=-3., a_max=3.)
            reg_labels_round = np.round(reg_labels_clip)

            cls_lables = np.zeros_like(reg_labels_round)
            cls_lables[reg_labels_round < 0] = 0
            cls_lables[reg_labels_round >= 0] = 1
        elif num_classes == 3:
            cls_lables = np.zeros_like(reg_labels)
            cls_lables[reg_labels < 0] = 0
            cls_lables[reg_labels == 0] = 1
            cls_lables[reg_labels > 0] = 2
        elif num_classes == 5:
            reg_labels_clip = np.clip(reg_labels, a_min=-2., a_max=2.)
            reg_labels_round = np.round(reg_labels_clip)

            cls_lables = np.zeros_like(reg_labels_round)
            cls_lables[reg_labels_round == -2.] = 0
            cls_lables[reg_labels_round == -1.] = 1
            cls_lables[reg_labels_round == 0.] = 2
            cls_lables[reg_labels_round == 1.] = 3
            cls_lables[reg_labels_round == 2.] = 4
        elif num_classes == 7:
            reg_labels_clip = np.clip(reg_labels, a_min=-3., a_max=3.)
            reg_labels_round = np.round(reg_labels_clip)

            cls_lables = np.zeros_like(reg_labels_round)
            cls_lables[reg_labels_round == -3.] = 0
            cls_lables[reg_labels_round == -2.] = 1
            cls_lables[reg_labels_round == -1.] = 2
            cls_lables[reg_labels_round == 0.] = 3
            cls_lables[reg_labels_round == 1.] = 4
            cls_lables[reg_labels_round == 2.] = 5
            cls_lables[reg_labels_round == 3.] = 6 
    ...
    def __getitem__(self, index):
        if (self.mode == 'train') and (index == 0):
            ...
            missing_rate = [np.random.uniform(size=(len(self.data[self.mode]['regression_labels']), 1)) for i in range(3)]
            ...
            if self.do_missing:
                ...
                missing_rate = [np.zeros((len(self.data[self.mode]['regression_labels']), 1)) for _ in range(3)]
        ...
        sample = {
            ...
            'rawText': self.rawText[index]
        }
        """
        sample = {
            'text': torch.Tensor(self.text_m[index]),
            'audio': torch.Tensor(self.audio_m[index]),
            'vision': torch.Tensor(self.vision_m[index]),
            'index': index,
            'id': self.ids[index],
            'labels': {k: torch.Tensor(v[index].reshape(-1)) for k, v in self.labels.items()},
            'rawText': self.rawText[index]
